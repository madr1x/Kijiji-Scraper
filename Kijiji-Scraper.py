...
def MailAd(ad_dict, email_title):  # Sends an email with a link and info of new ads
	...
	#------------------------------------------
	sender = 'morelogik999@gmail.com'
        passwd = ''
	receiver = 'morelogik999@gmail.com'
	...
	try:
		for ad_id in ad_dict:
			...
			body += ad_dict[ad_id]['Image'].decode('ascii', 'ignore') + '</a>' # UnicodeDecodeError: 'ascii' codec can't decode byte 0xc3 in position 49: ordinal not in range(128)
	...
	body += u'<p>This is an automated message, please do not reply to this message.</p>'
	...
	try:
		...
		raise
	try:
		server.sendmail(sender, receiver, msg.as_string().encode('utf-8', 'ignore'))
		server.quit()
...
def scrape(url, old_ad_dict, exclude_list, filename):  # Pulls page data from a given kijiji url and finds all ads on each page
	...
	else: # prints if the dict is empty
		print("did not send or write")
...
def main(): # Main function, handles command line arguments and calls other functions for parsing ads
	args = sys.argv
	if args[1] == '-h' or args[1] == '--help' or args[1] == '': # Print script usage help
